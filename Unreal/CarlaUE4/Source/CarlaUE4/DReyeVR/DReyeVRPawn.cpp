...
int32 ADReyeVRPawn::ReadSignalFile()
{
...
void ADReyeVRPawn::WriteData()
{
	...
	int32 Ignore = UKismetStringLibrary::Conv_StringToInt(UKismetStringLibrary::GetSubstring(Result, Result.Find(TEXT("IGNORE:"), ESearchCase::IgnoreCase, ESearchDir::FromStart) + 8, 1));

	// Only if the ignore is false, write the data to the CSV file
	if (Ignore == 0) {
		// Retrive the identifier information for the CSV data logging
		...
		FString Participant_ID = UKismetStringLibrary::GetSubstring(Result, Result.Find(TEXT("PARTICIPANT_ID:"), ESearchCase::IgnoreCase, ESearchDir::FromStart) + 16, 3);
		FString Trial_No = UKismetStringLibrary::GetSubstring(Result, Result.Find(TEXT("TRIAL_NO:"), ESearchCase::IgnoreCase, ESearchDir::FromStart) + 10, 1);

		// CSV data file paths
		const FString RT_Path = FPaths::ProjectContentDir() / TEXT("DataFiles/ReactionTimes.csv");
		const FString SWA_Path = FPaths::ProjectContentDir() / TEXT("DataFiles/SteeringWheelAngles.csv");
		const FString SWV_Path = FPaths::ProjectContentDir() / TEXT("DataFiles/SteeringWheelVelocity.csv");
		const FString BI_Path = FPaths::ProjectContentDir() / TEXT("DataFiles/BrakingInput.csv");

		// Generating the string for the first 4 common columns
		FString IdentifierColumns = FString::Printf(TEXT("\n%s, %s, %s, %s"), *Participant_ID, *RSVP, *TTS, *Trial_No);

		// Generating string for reaction times
		FString RT_Data = FString::Printf(TEXT("%s, %f"), *IdentifierColumns, ReactionTime);

		// Generating string for Steering Wheel Angles
		FString SWA_Data = FString::Printf(TEXT("%s, "), *IdentifierColumns);
		for (int32 i = 0; i < SteeringWheelAngles.size(); i++)
		{
			if (i == SteeringWheelAngles.size() - 1)
			{
				SWA_Data.Append(FString::Printf(TEXT("%f"), SteeringWheelAngles[i]));
			}
			else
			{
				SWA_Data.Append(FString::Printf(TEXT("%f, "), SteeringWheelAngles[i]));
		...
		// Generating string for Steering Wheel Velocities
		FString SWV_Data = FString::Printf(TEXT("%s, "), *IdentifierColumns);
		for (int32 i = 0; i < SteeringWheelVelocity.size(); i++)
		{
			if (i == SteeringWheelVelocity.size() - 1)
			{
				SWV_Data.Append(FString::Printf(TEXT("%f"), SteeringWheelVelocity[i]));
			}
			else
			{
				SWV_Data.Append(FString::Printf(TEXT("%f, "), SteeringWheelVelocity[i]));
		...
		// Generating string for Breaking Inputs
		FString BI_Data = FString::Printf(TEXT("%s, "), *IdentifierColumns);
		for (int32 i = 0; i < BrakingInput.size(); i++)
		{
			if (i == BrakingInput.size() - 1)
			{
				BI_Data.Append(FString::Printf(TEXT("%f"), BrakingInput[i]));
			}
			else
			{
				BI_Data.Append(FString::Printf(TEXT("%f, "), BrakingInput[i]));
		...
		// Writing data: Reaction times
		FFileHelper::SaveStringToFile(RT_Data, *RT_Path, FFileHelper::EEncodingOptions::AutoDetect, &IFileManager::Get(), EFileWrite::FILEWRITE_Append);

		// Writing data: Steering Wheel Angles
		FFileHelper::SaveStringToFile(SWA_Data, *SWA_Path, FFileHelper::EEncodingOptions::AutoDetect, &IFileManager::Get(), EFileWrite::FILEWRITE_Append);

		// Writing data: Steering Wheel Velocities
		FFileHelper::SaveStringToFile(SWV_Data, *SWV_Path, FFileHelper::EEncodingOptions::AutoDetect, &IFileManager::Get(), EFileWrite::FILEWRITE_Append);

		// Writing data: Braking Inputs
		FFileHelper::SaveStringToFile(BI_Data, *BI_Path, FFileHelper::EEncodingOptions::AutoDetect, &IFileManager::Get(), EFileWrite::FILEWRITE_Append);
	}
}
...
#if USE_LOGITECH_PLUGIN
	bIsLogiConnected = LogiIsConnected(WheelDeviceIdx); // get status of connected device
	// Check is signal to record handover performance is given
	int32 signal = ReadSignalFile();
	if (signal == 1 && !bRecordData)
	{
		bRecordData = true;
		// Start recording the reaction time. Then subtract it from the current time.
		ReactionTime = World->GetTimeSeconds();
		const FLinearColor MsgColour = FLinearColor(0, 1, 1, 1); // GREEN
		UKismetSystemLibrary::PrintString(World, FString::Printf(TEXT("TOR issued, RT_init:%f"), ReactionTime), true, false, MsgColour, 10.f);
	}
	else if (signal == 2)
	{
		bRecordData = false;
...
void ADReyeVRPawn::LogitechWheelUpdate()
{
	...
	const float WheelRotationDegree = (FMath::Clamp(float(WheelState->lX), -32767.0f, 32767.0f) / 32767.0f) * 900;
	...
	else
	{
		...
		bool isSignificantInput = FMath::IsNearlyEqual(WheelRotation, WheelRotationLast, LogiThresh) &&
		...
		if (EgoVehicle->GetAutopilotStatus() /* && isSignificantInput*/)
		{
		...
		// Record the handover performance during the take over
		if (bRecordData && !bRecordingOver)
		{
			UE_LOG(LogTemp, Log, TEXT("LHP: TOR Started - logging handover performance."));
			// Check if the driver reacted to TOR. If yes, log reaction time (Gold et. al)
			if (!bReactedToTOR && (abs(WheelRotationDegree) >= 2.f || BrakePedal >= 0.1f))
			{
				ReactionTime = World->GetTimeSeconds() - ReactionTime;
				const FLinearColor MsgColour = FLinearColor(0, 1, 1, 1); // GREEN
				UKismetSystemLibrary::PrintString(World, FString::Printf(TEXT("Reacted to TOR, RT:%f"), ReactionTime), true, false, MsgColour, 10.f);
				bReactedToTOR = true;

			}
			// log the data for each interval ONCE TOR is issued.
			if (World->GetTimeSeconds() >= FutureRecordingTimeStamp)
			{
				UE_LOG(LogTemp, Log, TEXT("LHP: Logged data at %d"), World->GetTimeSeconds())
					// Record the new data
					SteeringWheelAngles.push_back(WheelRotationDegree);
				float delta_WheelRotationRadians = (WheelRotationDegree - WheelRotationDegreeLast) * M_PI / 180.0;

				float SteeringWheelAngularVelocity = delta_WheelRotationRadians / RecordingTimeInterval;
				SteeringWheelVelocity.push_back(SteeringWheelAngularVelocity);

				BrakingInput.push_back(BrakePedal);

				// Update future time stamp
				FutureRecordingTimeStamp += RecordingTimeInterval;
			}
		}
		// If TOR is done, then write the data to the file once.
		else if (!bRecordData && bRecordingOver && !bDataIsWritten)
		{
			UE_LOG(LogTemp, Log, TEXT("LHP: Data written in CSV files"));
			// Write data to file if recording is over.
			WriteData();
			// Only Write the data to the csv file once.
			bDataIsWritten = true;
		}
	}
	...
	WheelRotationDegreeLast = WheelRotationDegree;
